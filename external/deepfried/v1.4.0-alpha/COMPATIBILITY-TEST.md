# Deep Fried Chicken 1.4.0 alpha compatibility test
...
6. Begin with one pass, Full neural work scale, Texture Boost off, and Preserve Native Tone & Color
...
- A late-loaded valid host must be found by continuing module-generation or
  fallback discovery. A renamed, weak export match is accepted only when it is
  unique; ambiguity must leave native output untouched.
...
- With a game that exposes Frame Generation, test one pass with FG off and FG
  on. NGX feature 11 must be forwarded untouched; Chicken processes only the
  native DLSS base frames.
- Confirm the exact Streamline boundary is acquired before testing. Exercise
  ...
  resolver/setter pointer before Chicken loaded. The dynamic route should log
  `Streamline DLSS-G option boundary wrapped`; the pre-cached fallback should
  log `Streamline process-lifetime detour installed` for the resolver and/or
  setter. A per-viewport mode, multiplier, buffer, or allocation-topology
  change should then log `Streamline Frame Generation topology transition` as
  armed before the topology Present. No wait or queue drain should occur inside
  the setter.
- The next pre-Present should log `Frame Generation pre-Present drain
  complete`, proving submitted Chicken work was fenced and its queue references
  detached before Streamline replaced the presentation topology.
- After the replacement Present, rearm is allowed only after terminal
  Reset/destroy proof and any final queue fence. Expect `Frame Generation
  replacement Present settled` with the pipeline rearmed for lazy creation.
  A waiting-for-Reset/destroy line may appear first; a blocked line is a test
  failure and requires restart.
- Exercise a game that destroys or resizes its primary swapchain before it
  publishes `slDLSSGSetOptions`. Only the last successfully presented primary
  swapchain should arm this path, and it should log `Frame Generation
  pre-destruction drain complete` before the host teardown returns. Destroying
  a probe or unrelated swapchain must not gate Chicken.
- While FG is on, switch DLAA -> Quality -> Performance -> DLAA without a
  restart. The last-valid contract and success counters must continue updating,
  and generated-frame presentation must not stall behind Chicken's lock.
- The tab should report `Frame Generation: detected and forwarded untouched`
  and the log should contain `native Frame Generation feature 11 observed`.
- This matrix is required validation for 1.4.0. Broad Frame Generation
  support remains WIP; the transition mechanism is not a claim that it has
  been live-validated in every game.
...
- Turn Ray Reconstruction off for the Chicken test and select native DLSS
  Quality or DLAA. Chicken 1.4.0 attaches to feature 1, not feature 13.
...
- Frame Generation transition: dynamic acquisition should log `Streamline
  ...
  `Streamline process-lifetime detour installed` for the resolver and/or
  setter. Both routes must be caught before the topology Present and continue
  with `Frame Generation pre-Present drain complete` and `Frame Generation
  replacement Present settled`. Native feature 11 should separately log
  `native Frame Generation feature 11 observed`; it remains game-owned and
  forwarded untouched.
- When the game tears down the primary topology before SetOptions, expect
  `Frame Generation pre-destruction drain complete` before the destruction
  callback returns, followed by replacement-Present settlement evidence.
