...
NGX feature 11, Frame Generation, is also forwarded directly and untouched.
Chicken cooks only the real base frames produced by DLSS Super Resolution; it
does not intercept, duplicate, or recursively process generated frames. Frame
Generation detection is reported in the tab and log for diagnosis.

For Streamline DLSS-G, 1.4.0 catches both the exact dynamic
`GetProcAddress(sl.interposer.dll, "slGetFeatureFunction")` route and
process-lifetime entry detours for a pre-cached `slGetFeatureFunction` or its
resolved feature-1000 `slDLSSGSetOptions`. Only that exact setter is wrapped.
A per-viewport mode, multiplier, buffer, or allocation-topology change gates
new Chicken recording before the genuine setter runs and before the topology
Present. At the next ReShade pre-Present boundary, Chicken proves prior
submissions, fences their D3D12 queues, and detaches only its queue references
before Streamline changes the presentation topology. After the replacement
Present, terminal command-list Reset/destroy proof and a final queue fence are
required before the old graph is reclaimed and neural work is lazily rearmed.
Missing proof fails closed and requires a restart. This remains a WIP
compatibility mechanism, not a claim of live Frame Generation validation
across games. Establish the game's stable FG-off DLSS/NR baseline first, then
test Frame Generation separately for that title.

Some games destroy or resize the last successfully presented primary
swapchain before publishing `slDLSSGSetOptions`. The primary
`destroy_swapchain` callback is therefore a second pre-destruction boundary:
Chicken gates new recording, fences and detaches its tracked queues before the
host teardown returns, and still defers graph reclamation and rearm until the
replacement finish-Present proves the old command stream terminal.
...
The add-on must be loaded before the game resolves native DLSS. In the
`ReShade.ini` beside the game executable, add it to the existing `[ADDON]`
section:
...
The dedicated tab provides the same operation under **Compatibility /
Startup**. Its add/remove button edits only Chicken's exact token, preserves
other comma/semicolon-separated entries and INI sections, and clearly requires
a full game restart.
...
keeps each feature keyed by `(hook slot, handle)`, and continues bounded
module-generation/fallback discovery for late-loaded hosts. Forwarder aliases
...
the active 1.4.0 release. Their old config/preset keys are ignored during
...
`Clean Fry (Experimental)` is an optional final spatial guard for two or more
...
| --- | --- | --- |
| D3D12 with native DLSS Super Resolution | Built in; same-output mode/DRS changes apply live; output/device changes use a fenced Chicken-only rebuild; Streamline DLSS-G transitions catch exact dynamic and pre-cached resolver/setter entry points before the topology Present, then use a pre-Present/post-Present drain contract; Frame Generation feature 11 and Ray Reconstruction/DLSSD feature 13 remain forwarded untouched | DLAA validated at 4K/RTX 4080 Super; Performance through Ultra Quality and 16:9/ultrawide/super-ultrawide contracts tested; broad Frame Generation support remains WIP and requires live per-game validation |
...
The optional **Neural work scale** control keeps the display/output contract
unchanged while running Chicken's private feature-18 graph at full resolution
or at `ceil(display/2)` in each dimension. Half resolution is an explicit
performance/quality trade-off, not a new native DLSS mode, and any scale change
uses the same fenced Chicken-only recreation path.
...
..\..\release\v1.4.0-alpha\deep-fried-chicken.addon64
..\..\release\v1.4.0-alpha\deep-fried-chicken-nvngx.dll
..\..\release\v1.4.0-alpha\deep-fried-chicken.cfg
..\..\release\v1.4.0-alpha\dlss5-dx11-bridge.addon64
..\..\release\v1.4.0-alpha\README.md
..\..\release\v1.4.0-alpha\QUICK-START.md
..\..\release\v1.4.0-alpha\COMPATIBILITY-TEST.md
..\..\release\v1.4.0-alpha\FEEDER-COMPATIBILITY.md
..\..\release\v1.4.0-alpha\FEEDER-INTEROP-v1.md
..\..\release\v1.4.0-alpha\LICENSE-Deep-Fried-Chicken.md
..\..\release\v1.4.0-alpha\LICENSE-RenoDX.md
..\..\release\v1.4.0-alpha\LEGACY-API-ADAPTER.md
..\..\release\v1.4.0-alpha\LICENSE-Microsoft-Detours.md
..\..\release\v1.4.0-alpha\LICENSE-NIGos-dlss5-bridge.md
..\..\release\v1.4.0-alpha\LICENSE-ReShade.md
..\..\release\v1.4.0-alpha\LICENSE-dlss5-bridge-vulkan-shader.md
..\..\release\v1.4.0-alpha\README-dlss5-bridge.md
..\..\release\v1.4.0-alpha\SHA256SUMS.txt
```
...
```powershell
(Get-FileHash .\Deep-Fried-Chicken-v1.4.0-alpha.zip -Algorithm SHA256).Hash
```
