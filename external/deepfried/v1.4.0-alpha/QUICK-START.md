# Deep Fried Chicken 1.4.0 alpha - Quick Start
...
In the `ReShade.ini` beside the game executable, add this to its existing
`[ADDON]` section:
...
Alternatively, launch once, open **Deep Fried Chicken > Compatibility /
Startup**, press **Enable early load (recommended)**, then fully restart the
game. The button adds only this add-on and preserves the other early-load
entries. It can also remove only its own entry.
...
3. Check **Compatibility / Startup**. If early load is not configured, enable
   it there and fully restart before troubleshooting a `waiting for native
   DLSS` status.
...
10. Optional: at two or more passes, open **Clean Fry (Experimental)** and
...
**Neural work scale** defaults to Full resolution. Half resolution runs the
private neural graph at `ceil(display/2)` while reconstructing to the unchanged
display extent. It trades neural detail for lower GPU/VRAM cost and uses a
fenced Chicken-only graph recreation when changed.
...
- Adds full- or half-resolution private neural work without changing the
  display contract. Scale, output-size, and device changes use the fenced
